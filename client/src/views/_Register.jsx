...
const Register = () => {
  ...
  const handleForm = (event) => {
    ...
    if (!user.email.trim() || !user.password.trim() || user.rol === 'Seleccione un rol' ||  user.lenguage === 'Seleccione una forma de compra') 
    {
    ...
    <form onSubmit={handleForm} className='col-10 col-sm-6 col-md-3 m-auto mt-5 App'>
      ...
      <div className='form-group mt-1'>
        ...
        <select
          ...
          className='form-select'
        >
          <option disabled>Seleccione donde compra plantas</option>
          <option value='Huertos'>Huertos comunales</option>
          <option value='Trueque'>Comunidades Trueque</option>
          <option value='Sociales'>Intercambios Sociales</option>
          <option value='Minorista'>Compra al detalle</option>
        </select>
      </div> */}
